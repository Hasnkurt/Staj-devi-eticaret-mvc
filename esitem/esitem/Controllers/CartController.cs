    ...
    public class CartController : Controller
    {
        ...
        private void SaveOrder(Cart cart,ShippingDetails model)
        {
            ...
            foreach (var item in cart.CartLines)
            {
                ...
                orderline.Price = item.Quantity * item.Ürün.Price;
                orderline.ÜrünId = item.Ürün.Id;
        ...
        public ActionResult Checkout(ShippingDetails model)
        {
            ...
            {
                ModelState.AddModelError("Ürün Yok", "Sepetenizde Ürün Bulunamamaktadır..");
            }
            if (ModelState.IsValid)
            {
                SaveOrder(cart, model);
                cart.Clear();
                return View("SiparinizTamamlandı");
            }
            else
            {
                return View(model);
        ...
        {
            var ürün = db.Ürüns.FirstOrDefault(i => i.Id == Id);
            if (ürün!=null)
            {
                GetCart().AddProduct(ürün, 1);
            }
        ...
        public PartialViewResult SepetÖzet1()
        {
            return PartialView(GetCart());
        }
        public ActionResult ÜrünSil(int id)
        {
            var ürünler = db.Ürüns.FirstOrDefault(i => i.Id == id);
            if (ürünler != null)
            {
                GetCart().SilÜrün(ürünler);
            }
            return RedirectToAction("Index");
        }
